...
app.controller('cook_register', ['$scope', '$http', '$route', '$location', '$cookieStore', '$timeout', '$base64', '$window', '$rootScope', 'cfpLoadingBar', 'Notification', '$localStorage', '$routeParams', 'blockUI', function ($scope, $http, $route, $location, $cookieStore, $timeout, $base64, $window, $rootScope, cfpLoadingBar, Notification, $localStorage, $routeParams, blockUI) {
    ...
    //FETCHING COOK COMMISSION INFO

    $scope.cook_commission_data = {};
    $scope.fetch_cook_commission = function () {

        $scope.u = {};
        $scope.u.cook_id = $cookieStore.get('cook_logged_in');
        $http({
            method: "POST",
            url: 'cook/fetch-commission-info-cook',
            data: $scope.u
        }).then(function mySucces(response) {

            console.log('cook commission');
            console.log(response);
            $scope.cook_commission_data = response.data[0];
    ...
    $scope.update_cook_commission = function (val) {
        ...
        if (val == 'accept') {
            ...
            swal({
                title: "Are you sure?",
                text: "You Are Going To Accept Your Commission!",
                type: "warning",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        $scope.u = {};
                        $scope.u.cook_id = $cookieStore.get('cook_logged_in');
                        $scope.u.status = 'accept';
                        $http({
                            method: "POST",
                            url: 'cook/commission-approved-by-cook',
                            data: $scope.u
                        }).then(function mySucces(response) {
       ...
       swal({
                title: "Are you sure?",
                text: "You Are Going To Reject Your Commission!",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
                confirmButtonText: "Yes, I Deny !",
                cancelButtonText: "No, cancel plz!",
                closeOnConfirm: true,
                closeOnCancel: true
            },
...
app.controller('user_register', ['$scope', '$http', '$location', '$interval', '$cookieStore', '$timeout', '$routeParams', '$base64', '$rootScope', 'cfpLoadingBar', 'Notification', '$route', '$localStorage', '$window', 'blockUI', '$crypthmac', function ($scope, $http, $location, $interval, $cookieStore, $timeout, $routeParams, $base64, $rootScope, cfpLoadingBar, Notification, $route, $localStorage, $window, blockUI, $crypthmac) {
    ...
    $scope.verify_user_otp = function (data) {
        ...
        if (parseInt(data.otp) == $localStorage.otp_val) {
            ...
            console.log('FINAL DATA AFTER OTTP VALID');
            console.log($scope.u);
                ...
                navigator.geolocation.getCurrentPosition(function (p) {
                    ...
                    for (var i = 0; i < $localStorage.lat_long_coll.length; i++) {
                        ...
                        markerB[i] = new google.maps.Marker({
                            ...
                            map: map,
                            icon: 'https://www.eatoeato.com/uploads/q.png',
    ...
    $scope.payment_gateway_wallet_online = function () {
        ...
        $scope.pay_model.surl = "https://www.eatoeato.com/user/payu-process-wallet-online";
        $scope.pay_model.furl = "https://www.eatoeato.com/user/payu-process-wallet-online";
    ...
    $scope.payment_gateway_test = function () {
        ...
        $scope.pay_model.surl = "https://www.eatoeato.com/user/payu-process";
        $scope.pay_model.furl = "https://www.eatoeato.com/user/payu-process";
    ...
    $scope.payment_gateway_wallet = function () {
        ...
        $scope.pay_model.surl = "https://www.eatoeato.com/user/payu-wallet-process";
        $scope.pay_model.furl = "https://www.eatoeato.com/user/payu-wallet-process";
...
app.controller('delivery_boy_controller', ['$scope', '$http', '$route', '$rootScope', '$routeParams', '$timeout', '$base64', 'cfpLoadingBar', 'Notification', '$cookieStore', '$location', '$localStorage', 'blockUI', function ($scope, $http, $route, $rootScope, $routeParams, $timeout, $base64, cfpLoadingBar, Notification, $cookieStore, $location, $localStorage, blockUI) {
    ...
    //$scope.dvpickuplistdata.length=0;
    ...
    $scope.drop_confirm_user_order2 = function (data) {

        $(".otp-popup").fadeIn();
        console.log('DEL BOY DDDD');
        console.log(data);
        var obj = {};
        obj.orderid = data.order_id;
        obj.code = data.delivery_id;
    ...
    $scope.show_user_drop_info_temp = function (data) {
        //href="#/deliveryboy/pickupdetails"
        $localStorage.delboyuserdata={};
        console.log(data.cook_data[0]);
        $localStorage.delboyuserdata.cookdata = data.cook_data[0];
        $localStorage.delboyuserdata.items = data.items;
        $localStorage.delboyuserdata.useraddr = data.useraddr_data[0].address;
        ...
        $localStorage.delboyuserdata.delivery_id = data.delivery_id;
    ...
    $scope.show_drop_order_info = function () {
        ...
        // //  var src="https://www.google.com/maps/search/"+$scope.view_dvcook_info.cook_latitude+","+$scope.view_dvcook_info.cook_longitude+"/@37.0625,-95.677068,4z/data=!3m1!4b1";
        // //   $scope.view_dvcook_info.srcinfo = src;
        document.getElementById('mapcanvas').src = "https://www.google.com/maps/embed/v1/place?key=AIzaSyC1PizRzTiLG9RF4pxVHdwxm7mHAuDETDc&q=" + $scope.view_dvuser_info.useraddr.latitude + "," + $scope.view_dvuser_info.useraddr.longitude
    ...
    $scope.loadCooks = function () {
        ...
        $http({
            ...
            for(var i=0;i<$scope.cooks_list_deatils.length;i++){
                ...
                if($scope.cooks_list_deatils[i].is_cook_commission_approved=='true'){
                      $scope.cooks_list_deatils[i].commission_status='PENDING';

                }
                 else if($scope.cooks_list_deatils[i].is_cook_commission_approved=='accept'){
                      $scope.cooks_list_deatils[i].commission_status='ACCEPTED';

                }
                else if($scope.cooks_list_deatils[i].is_cook_commission_approved=='reject'){
                      $scope.cooks_list_deatils[i].commission_status='REJECTED';

                }
                 else if($scope.cooks_list_deatils[i].is_cook_commission_approved=='false'){
                      $scope.cooks_list_deatils[i].commission_status='NOT SET';

                    ...
                    db.admin_infos.find({
                        ...
                        function (err, sdata) {
                            if (err || !sdata) console.log("No data found");
                                ...
                                var newobj={};
                                newobj._id="test";
                                newobj.info_title="Contact Us";
                                newobj.info_desc="test";
                                finalresobj.infodata.push(newobj);
...
router
    .post('/update-cook-by-id', function (req, res, next) {
        ...
        console.log(req.body);
        var cookcommission = parseFloat(req.body.cook_commission);
...

                {
                    ...
                    update: {
                        $set: {
                            ...
                            isDeactivate:req.body.isDeactivate,
        ...
        // SETTING COOK commission
        if (cookcommission > 0.0) {
            ...
            db.cook_infos.findAndModify(
                {
                    ...
                    update: {
                        $set: {
                            is_cook_commission_approved: 'true',
                    ...
                    update: {
                        $set: {
                            ...
                            'food_details.$.cuisine_list': req.body.update_food_details.cuisine_list,
                              'food_details.$.food_type': req.body.update_food_details.food_type,
            ...
            , function (err, cookarr) {
                ...
                db.order_infos.aggregate([
                    ...
                    },
                    {
                        $lookup: {
                            from: 'user_infos',
                            localField: 'items.addr_id',
                            foreignField: 'address.address_id',
                            as: 'useraddr_data'
                    ...
                    {
                        $project: {
                            'order_id': 1, 'is_picked': 1, delivery_id: 1, 'items.order_date': 1, 'time': 1, 'order_status': 1, 'cook_data.cook_name': 1, 'cook_data.cook_contact': 1, 'cook_data.street_address': 1, 'cook_data.landmark': 1, 'cook_data.cook_latitude': 1, 'cook_data.cook_longitude': 1, 'items.username': 1, 'items.addr_id': 1,'items.addr_info': 1, 'items.addr_name': 1, 'items.addr_contact': 1, 'items.addr_lat': 1, 'items.addr_long': 1, 'items.food_name': 1, 'items.food_price': 1, 'items.food_qty': 1, 'items.delivery_charge': 1, 'items.delivery_time': 1,'items.discount_amt': 1,
                            'items.pay_mode': 1, 'items.tax_rate': 1, 'items.food_type': 1, 'useraddr_data.address': 1
                        }
                    }
                ]
                ).sort({ 'items.order_date': -1 }, function (err, orderarr) {
                    ...
                    if (orderarr.length > 0) {
                        for (var i = 0; i < orderarr[0].items.length; i++) {
                            ...
                            a = parseFloat(orderarr[0].items[i].food_price) * parseFloat(orderarr[0].items[i].food_qty);
                            b = a + parseFloat(orderarr[0].items[i].delivery_charge);
                            b=b-parseFloat(orderarr[0].items[i].discount_amt);
                            tax = b * .18;
                        ...
                        for (var j = 0; j < orderarr[0].useraddr_data[0].address.length; j++) {

                                if( orderarr[0].useraddr_data[0].address[j].address_id== orderarr[0].items[0].addr_id){

                                        orderarr[0].useraddr_data[0].address= orderarr[0].useraddr_data[0].address[j];
                                }
